...
from src.domain.entities import Book, Author, BookAuthor, StructuredReadingPlan

from src.repositories import (
    AuthorRepository,
    BookAuthorRepository,
    BookRepository,
    StructuredReadingPlanRepository,
)
...
class TestReadingDatabase(unittest.TestCase):
    ...
    def test_01_book_repository(self):
        ...
        author = Author(first_name="Joe", last_name="Reis")
        ...
        book_authors = [
            BookAuthor(book_id=3, author_id=6, author_order=1),  # Reis, Joe
            BookAuthor(book_id=3, author_id=5, author_order=2),  # Housley, Matt
        ]
        authors = [author_repository.get_by_id(ba.author_id) for ba in book_authors]
        ...
        # Test structured reading plans
        structured_reading_plan_repository = StructuredReadingPlanRepository(
            "test_database.db"
        )

        # Read all structured reading plans
        structured_reading_plans = structured_reading_plan_repository.list_all()
        books_planned = [book_repository.get_by_id(srp.book_id) for srp in structured_reading_plans]
        print()
        for book in books_planned:
            print(book.id, book, )
        # # Create a structured reading plan
        # structured_reading_plan = StructuredReadingPlan(
        #     book_id=2,
        #     start_date=date(2026, 2, 16),
        #     target_end_date=date(2026, 2, 16) + timedelta(days=55),
        #     total_pages=552,
        #     plan_status="planned",
        #     plan_version=1,
        #     is_current=True,
        # )
        # structured_reading_plan.id = structured_reading_plan_repository.add(
        #     structured_reading_plan
        # )
        # self.assertEqual(structured_reading_plan.id, 1)
